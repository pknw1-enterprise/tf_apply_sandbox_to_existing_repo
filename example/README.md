...
# instructions
## Prepare the environment and module
...
module "sandbox" {
  ...
  commit_author                 = "Paul Kelleher"
  commit_email                  = "paul.kelleher@edfenergy.com"
...
![install](images/installing_workflows_with_terraform_module.gif)

<details>
  <summary>Running workflow install</summary>
...
- Each module (delivered as a YAML manifest and README) is delivered into the ```.githiub/workflows/``` folder under a new branch 
- Each can be reviewed and either rejected or applied
- A PR to merge each module into a single "installer" branch is raised during install

![Module Branches](images/branches_for_workflows.png)

## Activate chosen workflows 
Once you have reviewd and closed the PR's for the workflows, you can now apply them to your repo by merging in the install PR

![Module Branches](images/pull_requests.png)
![Module Branches](images/workflow_folder.png)
![Module Branches](images/workflows_merged_into_pr.png)

## Setup required GitHub Secrets

- to deploy sandbox you will need Azure credentials and encryption key
- to use rover, you need credentials for Azure

| Variable name in code | Suggested GitHub Secrets Values | Info |
| --------------------- | ------------------------------- | ---- |
| ```ARM_SUBSCRIPTION_ID```   | ```${{ secrets.DEV_SUB }}```          |      |
| ```ARM_TENANT_ID```         | ```${{ secrets.DEV_TENANT}}```        |      |
| ```ARM_CLIENT_ID```         | ```${{ secrets.DEV_CLIENT_ID}}```     |      |
| ```ARM_CLIENT_SECRET```     | ```${{ secrets.DEV_CLIENT_SECRET }}```|      |
|```encryptionkey```         | ```${{ secrets.project_key}}```       |      |


[sign into az-cli with service principal](https://jiasli.github.io/azure-notes/aad/Service-Principal-cli.html)